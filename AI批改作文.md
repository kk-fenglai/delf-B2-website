...
- **Provider：DeepSeek**（`api.deepseek.com`，OpenAI 兼容协议，用 `openai` SDK 访问）
...
|---|---|---|---|
| FREE | 无 | 0 | 0 |
| STANDARD | DeepSeek V3 | 20 | 10 |
| AI | DeepSeek V3 | 50 | 15 |
| AI_UNLIMITED | DeepSeek V3 | 200 | 20 |

- 每次 regrade 计入月配额（真实产生 token 成本）
- 另有 hourly 30 次保护上限，防前端死循环或 token 泄漏烧账单
- `costUsd` 每条记录落库，供运营对账

> **关于计划差异化**：迁移到 DeepSeek 后模型单一，三档计划主要靠**配额**区分（20/50/200 篇/月）。未来加入 `deepseek-reasoner`（R1）后可复活"模型档位"升级路径。
