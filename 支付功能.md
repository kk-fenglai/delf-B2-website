...
> **完整定价表、EUR 锚定规则与利润测算见 [`定价标准.md`](./定价标准.md)。**

### 成本结构（单次订阅，以 AI 档 €11.99/月为例）

| 成本项 | 数值 |
|---|---|
| 支付通道费（Stripe EU ≈ 2.9% + €0.25） | €0.60 |
| DeepSeek AI 批改（30 篇 + 15 次口语满额） | ≈ €0.50 |
| 邮件 + DB + 带宽摊薄 | ≈ €0.65 |
| **总成本** | **≈ €1.75** |
| **毛利** | **€10.24（85%）** |

> 年付 €119.99（10 个月价），现金流 + 降低流失，毛利率更高。

### 定价（EUR 锚定，存 DB）

| Product | Price code | Months | 金额 EUR | 自动续费 |
|---|---|---|---|---|
| STANDARD | `STANDARD_1M` | 1 | €5.99 | ✅ |
| STANDARD | `STANDARD_12M` | 12 | €59.99 | ❌ |
| AI | `AI_1M` | 1 | €11.99 | ✅ |
| AI | `AI_12M` | 12 | €119.99 | ❌ |
| AI_UNLIMITED | `AI_UNLIMITED_1M` | 1 | €16.99 | ✅ |
| AI_UNLIMITED | `AI_UNLIMITED_12M` | 12 | €169.99 | ❌ |

另有 USD/CNY 参考价（`STANDARD_1M_USD` 等），见 `pricing.js` 与 `定价标准.md`。
